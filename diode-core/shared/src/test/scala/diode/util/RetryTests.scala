...
object RetryTests extends TestSuite {
  ...
  def tests = Tests {
    ...
    "Backoff" - {
      ...
      r.getOrElse((policy, effect(policy)))._2.toFuture.flatMap { n =>
        println(s"First retry after ${System.currentTimeMillis() - now}")
        assert(System.currentTimeMillis() - now > 150)
        // next retry
        val rr = r.getOrElse((policy, effect(policy)))._1.retry(new Exception, effect)
        // check that effect happens in the future
        rr.getOrElse((policy, effect(policy)))._2.toFuture.map { n =>
