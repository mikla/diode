...
trait ModelRO[S] {
  ...
  /**
    ...
    */
  def zoom[T](get: S => T)(implicit feq: FastEq[? >: T]): NewR[T]
  ...
  /**
    ...
    */
  def map[F[_], A, B](f: A => B)(implicit ev: S =:= F[A], monad: Monad[F], feq: FastEq[? >: B]): NewR[F[B]] =
  ...
  /**
    ...
    */
  def flatMap[F[_], A, B](f: A => F[B])(implicit ev: S =:= F[A], monad: Monad[F], feq: FastEq[? >: B]): NewR[F[B]] =
  ...
  /**
    ...
    *   The function to apply
    */
  def zoomMap[F[_], A, B](fa: S => F[A])(f: A => B)(implicit monad: Monad[F], feq: FastEq[? >: B]): NewR[F[B]]
  ...
  /**
    ...
    *   The function to apply, must return a value of type `F[B]`
    */
  def zoomFlatMap[F[_], A, B](fa: S => F[A])(f: A => F[B])(implicit monad: Monad[F], feq: FastEq[? >: B]): NewR[F[B]]
}
...
trait ModelR[M, S] extends ModelRO[S] {
  ...
  /**
    ...
    *   The other reader
    */
  def zip[SS](that: ModelR[M, SS])(implicit feqS: FastEq[? >: S], feqSS: FastEq[? >: SS]): ModelR[M, (S, SS)]
}
...
trait BaseModelR[M, S] extends ModelR[M, S] {
  ...
  override def value: S = eval(root.value)

  override def zoom[T](get: S => T)(implicit feq: FastEq[? >: T]): ZoomModelR[M, T] =
    new ZoomModelR[M, T](root, get compose this.eval)

  override def zip[SS](that: ModelR[M, SS])(implicit feqS: FastEq[? >: S], feqSS: FastEq[? >: SS]): ZipModelR[M, S, SS] =
    new ZipModelR[M, S, SS](root, eval, that.eval)

  override def zoomMap[F[_], A, B](fa: S => F[A])(
      f: A => B
  )(implicit monad: Monad[F], feq: FastEq[? >: B]): ModelR[M, F[B]] =
...
class RootModelR[M <: AnyRef](get: => M) extends BaseModelR[M, M] {
  def root: RootModelR[M] = this

  override def eval(model: M): M = get
...
/**
  ...
  */
class ZoomModelR[M, S](val root: ModelR[M, M], get: M => S)(implicit feq: FastEq[? >: S]) extends BaseModelR[M, S] {
...
class ZipModelR[M, S, SS](val root: ModelR[M, M], get1: M => S, get2: M => SS)(implicit
  ...
  override def eval(model: M): (S, SS) = {
...
class RootModelRW[M <: AnyRef](get: => M) extends RootModelR(get) with BaseModelRW[M, M] {
  ...
  override def zoomRW[T](get: M => T)(set: (M, T) => M)(implicit feq: FastEq[? >: T]) =
...
/**
  ...
  */
class ZoomModelRW[M, S](root: ModelR[M, M], get: M => S, set: (M, S) => M)(implicit feq: FastEq[? >: S])
    extends ZoomModelR(root, get)
    with BaseModelRW[M, S] {
  override def updatedWith(model: M, value: S): M = set(model, value)
}
...
class MapModelRW[F[_], M, A, B](root: ModelR[M, M], get: M => F[A], f: A => B)(set: (M, F[B]) => M)(implicit
  ...
  override def updatedWith(model: M, value: F[B]): M = set(model, value)
}
